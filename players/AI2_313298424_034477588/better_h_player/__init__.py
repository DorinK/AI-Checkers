...
"""
This class implements the better player.
This player has both an advanced utility calculation that takes into account the value of certain board positions and
...
class Player(simple_player.Player):
    ...
    def utility(self, state):
        ...
        my_kings = 1 * piece_counts[KING_COLOR[self.color]]
        opponent_kings = 1 * piece_counts[KING_COLOR[opponent_color]]
    ...
    # This method is the same as the get_move method of the simple player but with a fix to the time management bug
    # found in the simple player, that does not count a move in case it is the only move thus making a mis-synchronization
    # between the internal time calculation of the class and the external time calculation - Ultimately resulting in
    # timeout
